...
// triggers fold transitions
  // backgrounds
  home.waypoint({
    handler: function(event, direction) {
      body.toggleClass('fold');}, offset: '-2%' });

  work.waypoint({
    handler: function(event, direction) {
      work.toggleClass('fold');}, offset: 0 });
  ...
  home.waypoint({
    handler: function(event, direction) {
      arrowb.toggleClass('out');}, offset: '-2%' });

  about.waypoint({
    handler: function(event, direction) {
      arrowc.toggleClass('out');}, offset: '60%' });

  contact.waypoint({
    handler: function(event, direction) {
      arrowd.toggleClass('out');}, offset: '60%' });
  ...
  // nav active item
    ...
    contact.waypoint({
      handler: function(event, direction) {
        if (liToggleEnabled) {
          navd.toggleClass('active');
        }
      }, 
      offset: 60 });

// automatically expands PVL
    var pvlStartTimer = $(window).load(function() { setTimeout(function() {
      $('nav .nava').addClass('hover');   
    }, 7000); })
    var pvlStopTimer = $(window).load(function() { setTimeout(function() {
      $('nav .nava').removeClass('hover');
    }, 10400); })


// disables smile loop when scrolled past header
    work.waypoint({
...
// applies arctext to header tagline
    var firstSmile,
        happyFace,
        winkyFace,
        happyFaceAgain,
    ...
    function init() {
      ...
      $("header .tagline h1 > span").arctext({radius: -1});

// triggers header tagline animation
      firstSmile = $(window).load(function() { setTimeout(function() {
        faceSmile();
      }, 2200); })
...
// triggers browser cycle animation
      cycleGo = $(function() {
        browserCycle();
      });
...
// triggers newsflash easter egg
    $('#dbugle-link').on('mouseup', function() {
      $('.news').addClass('in').addClass('flash');
    });
    $('#dbugle-content').on('mouseup', function() {
